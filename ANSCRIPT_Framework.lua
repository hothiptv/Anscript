...
local ANS_Folder = "ANSCRIPT"
local ConfigurationFolder = ANS_Folder.."/Configurations"
...
local ANS_Library = {
	...
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across ANSCRIPT
		...
		Light = {
			TextFont = "Gotham", -- Default will use the various font faces used across ANSCRIPT
...
local ANSCRIPT = game:GetObjects("rbxassetid://10804731440")[1]

ANSCRIPT.Enabled = false


if gethui then
	ANSCRIPT.Parent = gethui()
elseif syn.protect_gui then 
	syn.protect_gui(ANSCRIPT)
	ANSCRIPT.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	ANSCRIPT.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	ANSCRIPT.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == ANSCRIPT.Name and Interface ~= ANSCRIPT then
			Interface.Enabled = false
			Interface.Name = "ANSCRIPT-Old"
		end
	end
else
...
local Main = ANSCRIPT.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

ANSCRIPT.DisplayOrder = 100
...
local Notifications = ANSCRIPT.Notifications

local SelectedTheme = ANS_Library.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = ANS_Library.Theme[ThemeName]
	for _, obj in ipairs(ANSCRIPT:GetDescendants()) do
	...
	ANSCRIPT.Main.BackgroundColor3 = SelectedTheme.Background
	ANSCRIPT.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	ANSCRIPT.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	ANSCRIPT.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	ANSCRIPT.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	ANSCRIPT.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	ANSCRIPT.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if ANS_Library.Flags[FlagName] then
			spawn(function() 
				if ANS_Library.Flags[FlagName].Type == "ColorPicker" then
					ANS_Library.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if ANS_Library.Flags[FlagName].CurrentValue or ANS_Library.Flags[FlagName].CurrentKeybind or ANS_Library.Flags[FlagName].CurrentOption or ANS_Library.Flags[FlagName].Color ~= FlagValue then ANS_Library.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			ANS_Library:Notify({Title = "Flag Error", Content = "ANSCRIPT was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(ANS_Library.Flags) do
...
function ANS_Library:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= ANS_Library.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("ANSCRIPT | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
		...
		if ANSCRIPT.Name == "ANSCRIPT" then
...
function Hide()
	Debounce = true
	ANS_Library:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K", Duration = 7})
...
function ANS_Library:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "ANSCRIPT UI Framework"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= "ANSCRIPT UI Framework" then
		LoadingFrame.Version.Text = "ANSCRIPT UI"
	end
	...
	pcall(function()
		...
		if not isfolder(ANS_Folder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(ANS_Folder.."/Discord Invites") then
			makefolder(ANS_Folder.."/Discord Invites")
		end
		if not isfile(ANS_Folder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(ANS_Folder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"ANSCRIPT RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(ANS_Folder.."/Key System") then
			makefolder(ANS_Folder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("ANSCRIPT | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(ANS_Folder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(ANS_Folder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(ANS_Folder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						ANS_Library:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("ANSCRIPT | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					ANS_Library.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("ANSCRIPT | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			for _, Option in ipairs(DropdownSettings.Options) do
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("ANSCRIPT | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					ANS_Library.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("ANSCRIPT | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					ANS_Library.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("ANSCRIPT | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					ANS_Library.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("ANSCRIPT | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					ANS_Library.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function ANS_Library:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				ANS_Library:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

task.delay(3.5, ANS_Library.LoadConfiguration, ANS_Library)
if ANSCRIPT:FindFirstChild("Notice") then
	ANSCRIPT.Notice.Visible = true
	ANSCRIPT.Notice.Interact.MouseButton1Click:Connect(function()
		ANSCRIPT.Notice.Visible = false
	end)
end
